...
module.exports = {
  sendLink: async (req, res, next) => {
    ...
    try {
      ...
      if (!partialUser.userId)
        return res
          .status(409)
          .json({ message: "Désolé, cet email n'est pas enregistré" });
      if (partialUser.apisignup)
        return res
          .status(409)
          .json({
            message:
              "Ce compte a été créé à partir d'un compte Google, Github ou autre... Vous pouvez essayer de vous connecter avec ce compte",
          });
      ...
      await mailer.sendMail(
        user.email,
        "Réinitialisation du mot de passe Ikodi",
        `${resetPwdHtml.beginning}${resetLink}${resetPwdHtml.end}`
      );

      res
        .status(200)
        .json({
          message: `Un lien pour réinitialiser le mot de passe a été envoyé à ${user.email}, vous pouvez vérifier votre boite mail`,
        });
    } catch (error) {
  ...
  getResetPwd: async (req, res, next) => {
    ...
    if (!user)
      return res
        .status(409)
        .json({ message: "Désolé, ça n'a pas fonctionné. Contacter le sav." });
  ...
  resetPwd: async (req, res, next) => {
    ...
    if (password1 !== password2)
      return res
        .status(409)
        .json({ message: "Les mots de passe sont différents" });

    if (password1 === "" || password2 === "")
      return res.status(409).json({ message: "Au moins un champ est vide" });
    ...
    try {
      ...
      // Update the user
      user.password = hash;
      const newUser = await updateMePassword({
        id: user.id,
        newPassword: hash,
      });
